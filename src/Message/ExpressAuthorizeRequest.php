...
class ExpressAuthorizeRequest extends AbstractRequest
{
    ...
    public function getData()
    {
        ...
        $card = $this->getCard();
        if ($card) {
            $data['PAYMENTREQUEST_0_SHIPTONAME'] = $card->getShippingName();
            $data['PAYMENTREQUEST_0_SHIPTOSTREET'] = $card->getShippingAddress1();
            $data['PAYMENTREQUEST_0_SHIPTOSTREET2'] = $card->getShippingAddress2();
            $data['PAYMENTREQUEST_0_SHIPTOCITY'] = $card->getShippingCity();
            $data['PAYMENTREQUEST_0_SHIPTOSTATE'] = $card->getShippingState();
            $data['PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE'] = strtoupper($card->getShippingCountry());
            $data['PAYMENTREQUEST_0_SHIPTOZIP'] = $card->getShippingPostcode();
            $data['PAYMENTREQUEST_0_SHIPTOPHONENUM'] = $card->getShippingPhone();
